...
// 判斷跑馬燈字數
function checkMarqueeTextLength() {
  // 每當裡面的字數大於 12，就拿掉width 100%
  const marqueeArray = document.querySelectorAll(".marquee__box p");
  marqueeArray.forEach(function (el) {
    const msgInnerText = el.innerText.length;
    if (msgInnerText < 12) el.style.width = "100%";
  });
}
