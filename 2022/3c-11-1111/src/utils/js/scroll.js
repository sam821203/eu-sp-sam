...
// 錨點圓滑捲動，有jQ用jQ的，沒有就檢查有沒有原生的behavior smooth，有就用，沒有就用JS跑迴圈捲動的
export function smoothScroll ( top, left ) {
  var options = {
    "top": top || 0,
    "left": left || 0,
    "behavior": "smooth",
  ...
  if ( window.jQuery ) {
    // jQuery scrollTo
    $('html,body').stop().animate({ scrollTop: options.top, scrollLeft: options.left }, 500);
  } else if ( isSmoothScrollSupported ) {
    // Native smooth scrolling
    window.scrollTo( options );
  } else {
    var scrollingTo = function () {
      if ( window.scrollX != options.left || window.scrollY != options.top ) {
        setTimeout( function () {
          window.scrollTo( window.scrollX - 100, window.scrollY - 100 );
          scrollingTo();
        }, 0 );
      }
    }
