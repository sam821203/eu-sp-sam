...
export var initDailyTab = [buildDailyTab()];
export var dragOverflow = [buildDraggable()];
...
function buildDateTabItem(dateNow, index) {
  var dailyArray = dailyObject.oneDay2;
  var partDailyArray = sliceArrayByNumber(dailyArray, 7);
  var activeRound = Math.floor(index / 7);
...
function dateContentActive(index) {
  ...
  contentBox[index].classList.add("today");
...
function dailyTabReady(activeIndex) {
  ...
  var tabItems = Array.apply(
    null,
    tabBox.querySelectorAll(".tab__item--daily")
  );
  var tabContentBlock = document.getElementById("dailyTabContent");
  var contentBox = Array.apply(
    null,
...
function buildDraggable() {
  var slider = document.querySelector(".tab__zone");
  var preventClick = (e) => {
    e.preventDefault();
