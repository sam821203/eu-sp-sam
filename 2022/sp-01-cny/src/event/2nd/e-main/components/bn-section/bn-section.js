...
export var initBnSection2 = [
  setInterval( function () { chooseRandomBnItem() }, 1500 ),
  ScrollBnSlider(),
];
...
function chooseRandomBnItem() {
  ...
  promoItems.forEach( function (item) {
    // arrayAnimationClass.forEach( function (className) {
      item.classList.remove( "shakeY" );
    // } );
  });
  promoItems[chooseItemIndex].classList.add("shakeY");
...
function ScrollBnSlider() {
  ...
  window.addEventListener( "scroll", function () {
    ...
    // scrollItem2.style.transform = 'translateX(' + pos2 + '%)';    // scrollItem.style.transformOrigin = 'bottom';
