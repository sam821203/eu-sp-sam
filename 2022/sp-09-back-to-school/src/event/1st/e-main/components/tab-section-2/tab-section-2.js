...
function buildSliderContent(){
  if(!(dataTab2One && dataTab2Second && dataTab2Third)) return false;

  var dataArray = [dataTab2One, dataTab2Second, dataTab2Third];
...
function scrollTabSectionSecond() {
  ...
  window.addEventListener( "scroll", function () {
    ...
    var scrollPercentage2 = getScrollPercentage( scrollItem, 0, .8 );


    var pos = 150 + scrollPercentage * -150;

    var value = scrollPercentage * 1;
    var value2 = scrollPercentage2 * 1;

    scrollItem.style.transform = 'translateX(' + pos + '%) rotateX(' + pos + 'deg) rotateY(' + pos + 'deg) scale(' + value + ')';
    scrollItem2.style.transform = 'translateX(' + pos + '%) rotateX(' + pos + 'deg) rotateY(' + pos + 'deg) scale(' + value2 + ')';
