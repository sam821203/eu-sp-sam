...
export function createDateArray(originDate, length, intervalsDay) {
  ...
  while (index < length) {
    ...
    var dateItem = startDay.setTime(
      startTime + parseInt(index * intervalsDay * 24 * 60 * 60 * 1000)
    );
...
export function buildByCustomTime(
  timeSource,
  functionSource,
  timeInterval,
  countdownNumberArray,
  callBack
) {
  ...
  // FIXME:由於下方timer會自動重複，因此可以不用初始先跑一輪，故先關閉以增加效能(測試後若都正常可移除這部分)
  // 初始先執行過找出目前所在的區間
  ...
  var timer = setIntervalFix(function () {
    if (isNeedCountdownNumber) {
      ...
      window[countdownNumberArray[0]] = countDownNumber(
        timeSource[targetIndex],
        isDay
      );
    ...
    // 結束時處理
    if (targetIndex > timeSource.length - 1) {
...
// 倒數計時的function，最小到微秒，會回傳日時分秒微秒，參數：目標日期、是否顯示日期
...
export function setIntervalFix(callback, tick) {
  var countIndex = 1; // 計時任務執行次數
  var tick = tick || 1000; // 計時任務的時間間隙
