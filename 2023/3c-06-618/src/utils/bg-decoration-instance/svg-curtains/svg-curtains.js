...
local.init = function() {
  local.svg = document.getElementById('svgCurtains');

  $(window).on('resize',local.onResize);
  
  local.winWidth = $('#curtainsWrap').width();
  local.winHeight = $('#curtainsWrap').height();
...
local.createRectangle = function(i) {
  ...
  elem.setAttribute("width",Math.random()*5+$('#curtainsWrap').width()/10);
  elem.setAttribute("height", $('#curtainsWrap').height());
  TweenMax.set(elem,{x:(Math.random()*10+i*30), y:$('#curtainsWrap').height()* -0.02});
...
local.moveElement = function(elem,i) {
  var xDest = (Math.random()*$('#curtainsWrap').width()/3+i*30);
  var xDiff = $(elem).offset().left - xDest;
  
  console.log(xDiff);
  TweenMax.to(elem,Math.random()*6+1,{
    attr:{width:Math.random()*$('#curtainsWrap').width()/8+100},
    x:xDest,
    rotationZ:xDiff/50,
