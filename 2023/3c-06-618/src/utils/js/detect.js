...
export function laterDateShow(date, section) {
  // 採用沒到新月份就關掉的方式
  if (typeof date === "string") var checkPoint = new Date(date);
  else if (typeof date === "object") var checkPoint = date;

  if (isLaterThan(checkPoint)) {
    var sections = Array.apply(null, document.querySelectorAll(section));
...
// 判斷時間以補上"未到"和"過期"的 overlay
export function getDateFromElement(el) {
  return {
    now: new Date(),
...
export function getElementsArray(section, selector) {
  return Array.apply(null, document.querySelectorAll(section + ' ' + selector));
}

export function checkTimeForOverlay(section, overlayImage) {
...
export function checkActiveWithTime(section, selector) {
  var dateElements = getElementsArray(section, '[data-start]');
  var tabItemTopArray = getElementsArray(section, selector);

  dateElements.forEach(function (el, index) {
    var { now, start, end } = getDateFromElement(el);
    var dateElement = dateElements[index];
    var tabItemTopElement = tabItemTopArray[index];

    if (now >= start && now <= end) {
      dateElement.classList.add('active');
      tabItemTopElement.classList.add('active');
    } else if (now < start && index === 0) {
      tabItemTopElement.classList.add('active');
    } else {
      dateElement.classList.remove('active');
      tabItemTopElement.classList.remove('active');
    }
  })
}
