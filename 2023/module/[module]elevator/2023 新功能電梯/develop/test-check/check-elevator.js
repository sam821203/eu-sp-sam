...
(function () {
  ...
  // 如果是離線點開就直接建構離線電梯，如果不是ETmall也直接開離線電梯
  // 因為her執行fetch會被擋下來，因此isETmall要拉到fetch之前判斷
  // console.log( "isEtMall", isEtMall() );
  // TODO: 51行註解掉即可離線測試，記得上線打開
  if( isOffline() || !isEtMall() ) return buildLocalElevator();
  ...
  var getDataCheckElevator = setInterval(function () {
    ...
    if (typeof dataCheckElevator !== 'undefined') {
      ...
      // 先檢查是否在設定的時間內，是的話再繼續，否則結束判斷、回傳離線電梯
      // 因主視覺data-main-elevator.json有可能過於久遠而不存在(整個活動資料被後端伺服器清掉)，因此先確認是否在霸道電梯活動時間內再進行fetch
      isOlineDate = isDateOnline(startDate, dateNow, endDate);
      // console.log( "isOlineDate", isOlineDate );

      // 如果不在時間內就建構離線電梯
      if (!isOlineDate ) return buildLocalElevator();;
  
      // 主視覺data-main-elevator.json不存在的話，Fetch會報錯404，setInterval會回傳整個index.html，所以error會報『SyntaxError: Unexpected token '<', "<!DOCTYPE "... is not valid JSON』，所以會有兩行錯誤訊息
      const dataUrl = onlineElevatorUrl + 'data-main-elevator.json' + timeStamp;

      fetch(dataUrl)
        .then(response => {
          if (!response.ok) {
            // 找不到data-main-elevator這邊會報404
        ...
        // 找不到data-main-elevator這邊會報SyntaxError: Unexpected token '<', "<!DOCTYPE "... is not valid JSON』
  ...
  function initCheckElevator(json, elevatorMode, onlineElevatorUrl, timeStamp) {
    ...
    // 先檢查檢查是否是自動名單內或是mode核可名單，皆是的話就製作線上電梯，否則製作離線電梯
    ...
    if( bulidOnlineElevator) {
      // 如果有在名單內，加入線上左邊CSS，移除離線左邊CSS，最後加線上JS
