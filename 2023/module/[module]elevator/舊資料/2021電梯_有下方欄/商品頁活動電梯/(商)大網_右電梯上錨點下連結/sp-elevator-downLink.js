...
$(function () {
  ...
  function addFixedBottomNav() {
    ...
    // if (iconBottomNav.length <= 5) {
    for (var i = 0, iconLen = iconBottomNav.length; i < iconLen; i++) {

      var iconLength = iconLen,
        wordsLength = iconBottomNav[i].words.length,
        imgUndefined = iconBottomNav[i].imageUrl;
      //- 判斷icon位置(上方 || 左方)
      judgeIconPosition();
      fixedBottomNav += '<a href="' + iconBottomNav[i].linkUrl + '">';
      // console.log(iconBottomNav[i].imageUrl)
      //- 判斷有沒有圖片檔
      judgeWithImage();
      //- 判斷最小字數
      judgeTextLength();

      // fixedBottomNav += '<p>' + iconBottomNav[i].words + '</p>';
      fixedBottomNav += '</a></li>';
    }
    ;
    // }else {
    ...
    function judgeWithImage() {
      if (iconBottomNav[i].imageUrl.match(imgRegx) != null) {
        fixedBottomNav += '<img src="' + iconBottomNav[i].imageUrl + '" alt="">';
      }
      ;
    };
    ...
    function judgeTextLength() {
      var len = 4;
      // console.log(iconBottomNav[i].imageUrl.length)
      var noImageString = iconBottomNav[i].imageUrl.length;
      if (iconLength == 2 && wordsLength > (len + 4) && noImageString == 0) {
        // console.log('test')
        fixedBottomNav += '<p>' + iconBottomNav[i].words.substr(0, len + 4) + '</p>';
      }
      else if (iconLength == 2 && wordsLength > (len + 2) && noImageString > 0) {
        fixedBottomNav += '<p>' + iconBottomNav[i].words.substr(0, len + 2) + '</p>';
      }
      else if (iconLength == 3 && wordsLength > (len + 2)) {
        fixedBottomNav += '<p>' + iconBottomNav[i].words.substr(0, len + 2) + '</p>';
      }
      else if (iconLength >= 4 && wordsLength > len) {
        fixedBottomNav += '<p>' + iconBottomNav[i].words.substr(0, len) + '</p>';
      }
      else {
        fixedBottomNav += '<p>' + iconBottomNav[i].words + '</p>';
      }
  ...
  function spScrollRightNav(isOutLinkNav) {
    ...
    //- 只判斷大網有沒有值
    if (result.length == 0 && navContent_rightLink.length == 0) {
