...
import { buildDynamicLayout } from 'utils/js/build-layout.js'
...
// 指令分成兩大類，第一大類是各種與自己js中有關的流程設定，與dataSource較無關，有13種：
// outerTag、outerClass、isLazyLoad、imgLazyLoadClass、lazyLoadIndicatorSrc、
...
// outerClass: 決定最外層OuterTag的class name內容，注意如果isSlider有開則此項目會失效，要設定必須使用sliderClass，若沒有使用slider則可使用outerClass設定所需的class陣列
...
export function buildDynamicAutoSync(dataSource, customObject) {
  ...
  var outerClass = customObject.outerClass;
  var outerClassArray = detectStringOrArray( outerClass );
  var finalClassArray = isSlider ? sliderClassArray : outerClassArray;

  if ( Array.isArray( finalClassArray ) ) {
    finalClassArray.forEach( function ( el ) {
      outerElement.classList.add(el);
    });
  }

  // ============================== 建立DOM start ===================================
  // 如果有back物件，建立back的keys陣列傳進去可以篩選要生成的回饋選項，沒有的話就全生成(buildDynamicLayout預設四種key都會生成)
  if ( dataSource.back ) {
    var backArray = Object.keys( dataSource.back );
    var layoutDom = buildDynamicLayout(backArray);
  } else var layoutDom = buildDynamicLayout();
  // console.log( layoutDom );
  // ============================== 建立DOM end =====================================

  // ============================== 建立連結 start ===================================
  // a是結構的一環所以會在layout建好，name、img、price則在此建立
  var anchorElement = layoutDom.querySelector('a');
  ...
  // ======================== 組裝 tag start ============================
  var productImgWrap = layoutDom.querySelector( '.product__img' );
  var productNameWrap = layoutDom.querySelector( '.product__name' );
  var productPriceWrap = layoutDom.querySelector( '.product__price' );
  ...
  // 填入priceElement
  if ( isText ) {
    // TODO: dollars Sign的位置可能會是0或1 (1代表前面有類似「活動價」的文案)
    var priceSign = productPriceWrap.querySelectorAll( 'span' )[1];
    priceSign.style.display = 'none';
  }
  productPriceWrap.appendChild(priceElement);

  // 最後把整理好的layoutDom塞入outTag
  outerElement.appendChild(layoutDom);
  // ======================== 組裝 tag end ============================
