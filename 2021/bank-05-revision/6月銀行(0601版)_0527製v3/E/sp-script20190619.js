...
$(function () {
    ...
    function addLinkName() {
        ...
        navContent.forEach(function (el, index) {
            el.linkUrl += '?linkNameEv' + (index + 1);
        });
        navContentApp.forEach(function (el, index) {
            // var detectString = '%3FlinkNameEv' + (index + 1);
            var newLink = '';
            // judge 是否為縮網址
            var isNotAbbreviation = el.linkUrl.match("app.html");
            if (isNotAbbreviation) {
                newLink = el.linkUrl.replace('app.html', 'app.html');
                el.linkUrl = newLink;
            }
            else {
        ...
        navContent_brand.forEach(function (el, index) {
            el.linkUrl += '?linkNameEv' + (index + 20);
        })
    ...
    function spScrollNav() {
        ...
        function buildNav(isAppNav, isAppNav_brand) {
            ...
            navHtml += '<nav id="spFixedNav" class="sp-fixed__nav variation__right">';
            ...
            for (var i = 0; i < isAppNav_brand.length; i++) {
                ...
                if (i % 2 === 0) {
                    var getSearch2 = isAppNav_brand[i];
                    var getSearch3 = isAppNav_brand[i + 1];
                    navHtml += '<li class="sp-nav__2' + i + ' js-sp__nav__all sp-nav_g2">';
                    isThisPage();
                    navHtml += '</li>';
                }
                else if (isAppNav_brand.length % 2 != 0) {
                    console.error("銀行數量不對");
                }
                // 抓取網址相同 && 自己的網址切割後不是空值
            ...
            var $spNavLastLi = $("#spFixedNav").find('#navTop');
            ...
            function clickActive(obj, active) {
                if (active) {
                    return '<a data-dialog="' + obj.linkUrl + '" title="' + obj.words + '" target="' + obj.target + '" class="nav_bg2 js--openBigPhoto">' + obj.words + '</a>';
                } else {
                    return '<a data-dialog="' + obj.linkUrl + '" title="' + obj.words + '" target="' + obj.target + '" class="nav_bg2 js--openBigPhoto">' + obj.words + '</a>';
                }
            }
        ...
        function buildMobileNav(isAppNav, isAppNav_brand) {
            ...
            for (var i = 0; i < isAppNav_brand.length; i++) {
                ...
                navHtml += '<li class="sp-nav__2' + i + ' js-sp__nav__all sp-nav_g2">\
                                <a data-dialog="' + isAppNav_brand[i].linkUrl + '" title="' + isAppNav_brand[i].words + '" target="' + isAppNav_brand[i].target + '" class="nav_bg2 js--openBigPhoto">' + isAppNav_brand[i].words + '</a>\
                                </li>';
            ...
            function clickActive(obj, active) {
                if (active) {
                    return '<a data-dialog="' + obj.linkUrl + '" title="' + obj.words + '" target="' + obj.target + '" class="js--openBigPhoto active">' + obj.words + '</a>';
                } else {
                    return '<a data-dialog="' + obj.linkUrl + '" title="' + obj.words + '" target="' + obj.target + '" class="nav_bg2 js--openBigPhoto">' + obj.words + '</a>';
                }
            }
