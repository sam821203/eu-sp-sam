...
(function ($) {
  function buildVersion() {
    ...
    // 正式
    var cssLinkLeft = $(
      '<link type="text/css" href="https://media.etmall.com.tw/sp/12351/main-elevator-left.css?' +
        month + days + min + '" rel="stylesheet">'
    );
    var cssLinkRight = $(
      '<link type="text/css" href="https://media.etmall.com.tw/sp/12351/main-elevator-right.css?' +
        month + days + min + '" rel="stylesheet">'
    );

    $head.append(cssLinkLeft);
    $head.append(cssLinkRight);
    initBuild($head);
  }
...
function initBuild(el) {
  ...
  el.append(
    $.getScript(
      'https://media.etmall.com.tw/sp/12351/main-elevator.js?' +
        timeStamp
    )
  );
}
