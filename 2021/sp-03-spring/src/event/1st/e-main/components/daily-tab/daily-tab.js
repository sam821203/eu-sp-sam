...
var totalDailyTab = 21;
var lastHeadDateLength = 5;
...
function dailyTabChange() {
    ...
    var arrayDailyTabDate = createDateArray("2021/3/17 10:00", totalDailyTab)
...
function nowDateShow(i) {
    ...
    if (i >= totalDailyTab) return false;

    var contentBoxes = Array.apply(null, document.querySelectorAll('#dailyTabFirstContent .content__box'));
    ...
    // console.log(arrayDailyTabHead.length)
    if (i < 1) {
        arrayDailyTabHead[0].classList.add('daily-active');
        contentBoxes[0].classList.add('active');
    }
    // 視覺要留5個，故在低於5時候就不會刪除slide，所以不會小於5
    else if (arrayDailyTabHead.length == lastHeadDateLength) {
        var slideCount = i - (contentBoxes.length - lastHeadDateLength);
        var prevDateClose = i - 1;
        // console.log('slideCount' ,slideCount)
        // console.log('prevDateClose' ,prevDateClose)
...
function buildDailyTabContent(i) {
    ...
    tabItems.forEach(function (el, index) {
        el.addEventListener('click', function () {
            ...
            dailyTabFirstContentReady(index);

        });
    });

    // 初始化輪播(剛load進來預設第一個頁籤active)
    dailyTabFirstContentReady();

    // 初始化輪播content樣式
    var contentSlides = Array.apply(null, document.querySelectorAll('#dailyTabFirstContent .content__box .product--dynamic'));
...
function buildSliderContent() {
    ...
    var sliderContainerArray = Array.apply(null, document.querySelectorAll('#dailyTabFirstContent .swiper-wrapper'));
...
function dailyTabHeadReady(i) {
    ...
    var dailyTabHead = new Swiper('#dailyTabHead', {
        slidesPerView: lastHeadDateLength,
        speed: 600,
        observer: true,
    });
    ...
    // TODO:移除頁籤在此，不用就要關掉
    // 總天數 - 要留的標籤數
    var totalDailyTabIndex = totalDailyTab - lastHeadDateLength;
    if (index > 0 && index <= totalDailyTabIndex) {
...
// tab content 內容輪播
function dailyTabFirstContentReady(index) {
    var index = index || 0;

    var arrayTabFirstContents = Array.apply(null, document.querySelectorAll('.tab__slider--daily'));


    var sliderTabFirstContents = new Swiper(arrayTabFirstContents[index], {
        breakpoints: {
            960: {
                slidesPerView: 2,
            },
