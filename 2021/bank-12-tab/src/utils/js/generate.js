export function generateGlobalUXItems() {
  ...
  var basicScrollTop = function () {
    // 電梯可能會有其他的go top，設定全啟動由CSS啟閉
    var btnTops = Array.apply( null, document.querySelectorAll( '.js--go-top' ) );

    // 小網的要滑下來來才出現
    var btnMobileGoTop = document.getElementById( 'goTop' );
    // Reveal the button
    var btnReveal = function () {
      if (window.scrollY >= 300) {
        btnMobileGoTop.classList.add('is-visible');
      } else {
        btnMobileGoTop.classList.remove('is-visible');
      }
    }
    // Smooth scroll top
    ...
    btnTops.forEach( function ( el ) {
      el.addEventListener( 'click', topScrollTo );
    });
  };
  // append
  ...
  function checkElevatorGoTop () {
    var winWt = document.body.clientWidth;
    var elevatorGoTop = document.getElementById( 'elevatorGoTop' );
    var goTop = document.getElementById( 'goTop' );

    if ( winWt >= 1260 && elevatorGoTop ) goTop.style.display = 'none';
  }
  window.addEventListener('load', checkElevatorGoTop);
  window.addEventListener('resize', checkElevatorGoTop);
  ...
  if (window.jQuery) {
    $('a[href^="#"]').on('click', function (e) {
      ...
      if (target.length) {
        ...
        var navHeight = $( '#navWrap' ).height() > 0 ? $( '#navWrap' ).height() : progressBarHeight;
