...
export function buildItemGroupElement(dataSource, isLazyLoad, customDynamicArray, customLinkArray) {
  // 填連結確認來源是銷編且有填
  ...
  if (Array.isArray(customDynamicArray)) {
    ...
    var priceKey = 'price';
    var textKey = 'text';
    var sliderLiClass = 'build__item__5a1b4s';
    if(isLazyLoad) {
      var sliderImgLazyLoadClass = 'lazyload';
  ...
  var sliderLiElement = document.createElement('div');
  ...
  sliderWrap.classList.add('dayGroup');
  productImgWrap.classList.add('item__img');
  productNameWrap.classList.add('item__name');
  productPriceWrap.classList.add('item__price');
