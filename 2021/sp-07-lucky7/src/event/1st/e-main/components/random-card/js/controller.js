// 此檔案壓縮混淆後在asset處直接引入資料夾、由check.js判斷用script標籤引入，此處只放未壓縮前的程式碼備份
...
function loadObjData() {
  var checkDataObjLoad = setInterval(function () {
    ...
    if (typeof obj !== 'undefined') {
      ...
      // data obj沒有nr變數或是nr等於0是要重複、nr等於1或true是不重複
      if (typeof nr === 'undefined' || nr === 0 || nr === false) {
        arrayCardFronts = setRandomData(6);  // 重複
      } else {
        arrayCardFronts = setRandomData(6, true);  // 不重複
      }

      //TODO: dev用，打開知道最終pass出去的陣列，注意production版要關掉
      console.log(arrayCardFronts);
