...
function mainFunc() {
	...
	$(".sec03 .slide-box").on('afterChange', function () {
		...
		$(".sec03 .slick-current .gif").show().attr('src', gifUrl + '.gif');
	});
...
function detectiveBreakpoint(breakpoint) {
	var nowBP = breakpoint[0];

	$.each(breakpoint, function (idx, obj) {
	...
	if (Object.getOwnPropertyNames(nowBP)[0] === Object.getOwnPropertyNames(preBP)[0]) {
		return false;
	}
...
function mediaQuery(nowBP) {
	var nowBPName = Object.getOwnPropertyNames(nowBP)[0];
