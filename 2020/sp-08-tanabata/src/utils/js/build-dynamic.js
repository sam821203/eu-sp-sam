...
// 可以給custom陣列： customArray = [imgSrcKey, altKey, nameKey, priceKey, textKey, sliderLiClass]
// 決定要抓data中何種屬性當作目標連結，注意因為是陣列，順序要對，如果屬性不對會在console中報錯
// 最後會return包好的li element，可另外填入需要的ul element中
export function buildDynamicSliderLiElement(dataSource, customDynamicArray, customLinkArray) {
  // 填連結確認來源是銷編且有填
  ...
  if (Array.isArray(customDynamicArray)) {
    ...
    // console.log(imgSrcKey, altKey, nameKey, priceKey, textKey, sliderLiClass);
  ...
  buildProductNumberImg(dataSource, sliderImgElement);
