...
// 2. 正常只要給dataSource和targetImgElement，預設會自動檢查銷編key為productNumber的並填入，不管有沒有銷編key，
// 如果有會imgSrc就會走這邊、以及如果有imgAlt會填上img tag的alt屬性，沒有則預設抓name當alt
// 3. 預設在寬768以上用XL的圖片size、以下用L，要修改可以填入imgSizeArray：[windowWidth, webSize, mobileSize]
// 4. 可以給custom陣列： customArray = [productNumberKey, imgSrcKey, altKey]決定要抓data中何種屬性當作目標
// 連結，注意因為是陣列，順序要對，如果屬性不對會在console中報錯
// 5. 為了可以一次填src與alt，採用將目標img tag傳入function，而不用return資料的方式
export function buildProductNumberImg(dataSource, targetImgElement, imgSizeArray, customArray) {
  ...
  if (Array.isArray(customArray)) {
    ...
    var altKey = customArray[2] ? customArray[2] : 'imgAlt';
